...
{
	printf("Initialization, pleas wait...\n");
		...
		else if (!user.isLost())
		{
			...
			if (users[i].getSkeleton().getState() == nite::SKELETON_TRACKED && mindstrom_connection_open && user.getId() == 1)
			{
				...
				if(positions["right_hand"]['y'] > positions["right_shoulder"]['y']){
					...
					} else if(positions["right_hand"]['x'] < (positions["right_shoulder"]['x'] - precision)) {
